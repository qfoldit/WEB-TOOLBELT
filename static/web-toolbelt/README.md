...
**All Rights Reserved** — different from the AGPL-3.0 used by qFoldIT's
other toolbelts. See this directory's own `LICENSE` file for the exact
terms. In short: copying, modifying, and redistributing this specific
code (the `qfoldit.github.io/web-toolbelt/` sandbox) is restricted to
qFoldIT GitHub organization members and anyone with express written
permission — unlike `WEB-TOOLBELT` (the companion source repo, AGPL-3.0)
and the other toolbelts, which anyone may copy under copyleft terms.

This is a legal restriction, not a technical one: the page is still
served publicly at `qfoldit.github.io/web-toolbelt/`, so anyone can view
or run it as intended — the license restricts reuse/redistribution
rights, not the ability to load the page in a browser.
